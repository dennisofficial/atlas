...
export function orchestratorInstructions(args: {
  ...
  return [
    ...
    '',
    'Replying: the surface and surface id are exactly what the event header carries (e.g. surface github, id owner/repo#12 for an issue, owner/repo/pull/34 for a pull request). The control plane refuses any surface not aliased to this work item, rate-limits replies per work item, and its refusals come back as the tool error — read them. Your own replies never come back to you as events. When a human writes to you on a surface — a question, an instruction, a nudge — respond, every time, even if the response is one line acknowledging and saying you are still working. Silence reads as a hang to the person watching. Reserve not replying for bot chatter and for comments that are plainly a human talking to another human about something else.',
    '',
