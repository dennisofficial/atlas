...
export function implementerInstructions(args: {
  ...
  return [
    ...
    'You execute the work the orchestrator hands you, in the checkout that is your working directory, and you report back exactly once, through the factory_submit_result tool below.',
    '',
    ...
    '',
    'The git remote `origin` carries no credentials. For GitHub access, call the factory_git_token tool with the branch you will publish on; it returns a factory installation token.',
    '',
    ...
    '',
    'Never open a pull request, and never comment on GitHub: delivery is the orchestrator\'s guarded step after review, and reporting happens only through factory_submit_result.',
    '',
    ...
    '',
    '  1. Mint a token as above and export it: export GH_TOKEN=<the token factory_git_token returned>',
    '',
    '  2. Commit all local work, then run the helper below from the checkout root:',
    '',
    '     node /tmp/factory-push.mjs ' + args.repo + ' <branch> <expected-head-oid> "<conventional headline>"',
    '',
    '     <expected-head-oid> is the base SHA on the first publish and the OID the previous publish printed on revisions. On a first publish the helper creates the branch ref at that commit for you — a branch that already exists remotely is treated as done. The mutation refuses anything but the true remote head with STALE_DATA, which means the remote moved under you: fetch, reconcile, and retry rather than publishing blind. The helper prints the new commit\'s OID.',
    '',
    '  3. Point your checkout at the remote head, so the reviewer\'s snapshot of this drive matches the SHA you report:',
    '',
    '     git fetch https://x-access-token:$GH_TOKEN@github.com/' + args.repo + '.git <branch>',
    '     git reset --hard <oid-from-step-2>',
    '',
    '  4. Report the OID from step 2 as head_sha.',
    '',
    ...
    '  const headers = { authorization: `Bearer ${process.env.GH_TOKEN}`, \'content-type\': \'application/json\' }',
    '  const existing = await fetch(`https://api.github.com/repos/${repo}/git/ref/heads/${branch}`, { headers })',
    '  if (existing.status === 404) {',
    '    const created = await fetch(`https://api.github.com/repos/${repo}/git/refs`, {',
    '      method: \'POST\', headers, body: JSON.stringify({ ref: `refs/heads/${branch}`, sha: expected }),',
    '    })',
    '    if (!created.ok) { console.error(await created.text()); process.exit(1) }',
    '  } else if (!existing.ok) { console.error(await existing.text()); process.exit(1) }',
    ...
    '    } },',
    '  })',
    '  const res = await fetch(\'https://api.github.com/graphql\', { method: \'POST\', headers, body })',
    ...
    '',
    `When you are done — whether the work landed or you are blocked — submit your result by calling factory_submit_result with runId "${args.runId}" and the result object.`,
    '',
    ...
    '',
    'The control plane refuses results that do not match this contract, results whose branch is not on the remote at the reported SHA, and results from anyone but this run. Refusals come back as the tool error — read them, fix, and resubmit. A refused result is not recorded; your run stays open until one is accepted.',
    '',
...
export function reviewerInstructions(args: {
  ...
  return [
    ...
    '',
    `When you are done, submit your verdict by calling factory_submit_result with runId "${args.runId}" and the result object.`,
    '',
    'The result object, exactly this shape:',
    '',
    ...
    '',
    'Request changes only for defects worth a revision cycle — the loop allows at most two, and a frivolous cycle burns one. Refusals come back as the tool error — read them, fix, and resubmit.',
  ].join('\n')
}
