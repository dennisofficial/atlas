...
export function implementerInstructions(args: {
  ...
  return [
    ...
    '',
    '  The token authenticates GitHub API calls as `Authorization: Bearer $token` and git fetches over https://x-access-token:$token@github.com/' + args.repo + '.git. It lives about an hour; mint a fresh one per operation rather than saving it.',
    '',
    ...
    '',
    'The mutation models every change as full file contents, base64-encoded — binary files are fine. If the API publish genuinely cannot carry your change (a payload too large for one call, say), fall back to an unsigned `git push` over the token remote and record why in known_limitations — an unverified branch beats a broken one.',
    '',
